...
const ProductDetails = () => {
  ...
  const fetchProductDetails = async () => {
    setLoading(true);
    try {
      const response = await api.post("/product-details", { productId: params._id }); // Using centralized API client
        ...
        {loading ? (
          ...
          <div className="flex flex-col gap-1">
            ...
            <div className="flex items-center gap-2 text-xl lg:text-3xl font-medium my-2">
              <p className="text-red-700">{displayINRCurrency(data?.sellingPrice)}</p>
              <p className="text-slate-400 line-through">{displayINRCurrency(data?.price)}</p>
            </div>
            <div className="flex items-center gap-3 my-2">
              <button
                className="border-2 border-red-500 rounded px-3 py-1 min-w-[120px] text-red-600 font-medium hover:bg-red-600 hover:text-white"
                onClick={(e) => handleBuyProduct(e, data?._id)}
              >
                Buy
              </button>
              <button
                className="border-2 border-red-500 rounded px-3 py-1 min-w-[120px] font-medium bg-red-600 text-white hover:text-red-500 hover:bg-white"
